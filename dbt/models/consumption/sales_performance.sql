{{
  config(
    ...
    unique_key         = ['YEAR', 'MONTH', 'PRODUCT_CATEGORY', 'REGION', 'COUNTRY', 'SALES_CHANNEL', 'ORDER_PRIORITY']
  )
}}
...
WITH fact_base AS (
    ...
    {% if is_incremental() %}
        WHERE ORDER_DATE_KEY >= (
            SELECT TO_CHAR(CURRENT_DATE - INTERVAL '30 days', 'YYYYMMDD')::INTEGER
        )
    {% endif %}
),
...
enriched AS (
    SELECT
        ...
        dd.IS_WEEKEND
    FROM fact_base f
    JOIN {{ ref('dim_product') }}   dp  ON f.PRODUCT_ID   = dp.PRODUCT_ID
    JOIN {{ ref('dim_geography') }} dg  ON f.GEOGRAPHY_ID = dg.GEOGRAPHY_ID
    JOIN {{ ref('dim_channel') }}   dc  ON f.CHANNEL_ID   = dc.CHANNEL_ID
    JOIN {{ ref('dim_priority') }}  dpr ON f.PRIORITY_ID  = dpr.PRIORITY_ID
    JOIN {{ ref('dim_date') }}      dd  ON f.ORDER_DATE_KEY = dd.DATE_KEY
),
